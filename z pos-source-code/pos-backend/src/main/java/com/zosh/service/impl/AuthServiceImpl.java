...
//        update last Login
    ...
    @Override
    public String sendOTP(String email) throws UserException {
        User user = userRepository.findByEmail(email);
        if (user == null) {
        ...
        Optional<TwoFactorOTP> oldOtp = twoFactorOTPRepository.findByUserId(user.getId());
        oldOtp.ifPresent(twoFactorOTPRepository::delete);
        ...
        twoFactorOTPRepository.save(newOtp);

        System.out.println("OTP successfully generated for user: " + otp);
        emailService.sendEmail(email, "Your Login OTP", "Your login OTP is: " + otp + ". Do not share this with anyone.");

        return newOtp.getId();
    }

    @Override
    public AuthResponse verifyOTP(String id, String otp) throws Exception {
        TwoFactorOTP twoFactorOTP = twoFactorOTPRepository.findById(id)
                .orElseThrow(() -> new Exception("Verification token not found or expired"));
        ...
        // Generate authentication for the JWT provider using Custom user details
        ...
        AuthResponse response = new AuthResponse();
        response.setTitle("Login success via OTP");
        response.setMessage("Welcome Back " + user.getEmail());
        response.setJwt(jwt);
        response.setUser(UserMapper.toDTO(user));
