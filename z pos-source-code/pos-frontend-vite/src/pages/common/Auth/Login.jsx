...
const Login = () => {
  ...
  const { error, loading, otpId, otpSent } = useSelector((state) => state.auth)
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      handleAuthSuccess(resultAction)
    } catch (error) {
      toast({
        title: "Error",
  ...
  const handleAuthSuccess = (resultAction) => {
    if (login.fulfilled.match(resultAction) || verifyOtp.fulfilled.match(resultAction)) {
        toast({
          title: "Success",
  ...
  const handleSendOtp = async () => {
    ...
    try {
      ...
      if (sendOtp.fulfilled.match(resultAction)) {
        ...
        toast({ title: "Error", description: resultAction.payload || "Failed to send OTP", variant: "destructive" })
      }
    } catch (error) {
  ...
  const handleVerifyOtp = async () => {
    ...
    setIsLoading(true)
    try {
      const resultAction = await dispatch(verifyOtp({ id: otpId, otp: otpValue }))
      handleAuthSuccess(resultAction)
    } catch (error) {
      ...
      <div className="w-full lg:w-1/2 flex flex-col justify-between p-8 sm:p-16 relative z-10 bg-black/40 dark:bg-black/60 backdrop-blur-xl border-r border-white/20 shadow-2xl">
        ...
        <div className="flex justify-between items-center bg-transparent">
          <div className="flex items-center space-x-2 cursor-pointer" onClick={() => navigate('/')}>
            ...
            <span className="text-2xl font-bold tracking-tight text-[#f97316]">Pos ai</span>
          </div>
        ...
        <div className="w-full max-w-[400px] mx-auto flex-1 flex flex-col justify-center py-10">
          ...
          {emailSent ? (
            <div className="text-center animate-in fade-in slide-in-from-bottom-4 duration-500">
              ...
              <div className="space-y-4">
                ...
                <p className="text-sm text-foreground/80">
                  ...
                  <button onClick={() => setEmailSent(false)} className="text-[#f97316] hover:underline font-semibold">
                    try again
                  </button>
                </p>
              </div>
            ...
            <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
               <h1 className="text-[2.5rem] font-semibold mb-2 text-foreground drop-shadow-sm">Verify OTP</h1>
               <p className="text-muted-foreground mb-8">Enter the 6-digit code sent to your email.</p>
               
               <div className="flex justify-center mb-8">
                 <InputOTP maxLength={6} value={otpValue} onChange={setOtpValue}>
                   <InputOTPGroup className="gap-2">
                     <InputOTPSlot index={0} className="h-14 w-12 text-xl border-foreground/20 bg-background/30" />
                     <InputOTPSlot index={1} className="h-14 w-12 text-xl border-foreground/20 bg-background/30" />
                     <InputOTPSlot index={2} className="h-14 w-12 text-xl border-foreground/20 bg-background/30" />
                     <InputOTPSlot index={3} className="h-14 w-12 text-xl border-foreground/20 bg-background/30" />
                     <InputOTPSlot index={4} className="h-14 w-12 text-xl border-foreground/20 bg-background/30" />
                     <InputOTPSlot index={5} className="h-14 w-12 text-xl border-foreground/20 bg-background/30" />
                   </InputOTPGroup>
                 </InputOTP>
               </div>
               ...
               <Button
                  ...
                  className="w-full h-14 text-lg font-medium bg-[#f97316] hover:bg-[#ea580c] text-white transition-all shadow-[0_8px_30px_rgb(249,115,22,0.3)] hover:shadow-[0_8px_30px_rgb(249,115,22,0.5)] mt-2 border-0"
                  disabled={isLoading}
                >
                ...
                <button 
                  ...
                  className="w-full mt-6 text-sm text-foreground/60 hover:text-foreground transition-colors"
                >
              ...
              <form onSubmit={handleForgotPassword} className="space-y-6">
                <div>
                  <Input
                    ...
                    className="h-14 px-4 text-base focus-visible:ring-[#f97316] transition-all bg-background/30 backdrop-blur-sm border-foreground/20 shadow-inner placeholder:text-foreground/50"
                ...
                <div className="flex space-x-3 mt-8">
                  ...
                  <Button type="submit" className="flex-1 h-12 text-base bg-[#f97316] hover:bg-[#ea580c] text-white shadow-lg" disabled={loading}>
                    {loading ? (
            ...
            <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
              ...
              <form onSubmit={handleLogin} className="space-y-5">
                <div>
                  <Input
                    ...
                    className="h-14 px-4 text-base focus-visible:ring-[#f97316] transition-all bg-background/30 backdrop-blur-sm border-foreground/20 shadow-inner placeholder:text-foreground/50 hover:bg-background/40"
                    placeholder="Email"
                    required
                  />
                </div>
                ...
                <div className="relative">
                  <Input
                    ...
                    onChange={handleInputChange}
                    className="h-14 px-4 pr-12 text-base focus-visible:ring-[#f97316] transition-all bg-background/30 backdrop-blur-sm border-foreground/20 shadow-inner placeholder:text-foreground/50 hover:bg-background/40"
              ...
              <div className="mt-8 text-[15px]">
                ...
                <button
                  ...
                  className="text-orange-600 dark:text-orange-400 hover:text-orange-700 hover:underline inline-block font-medium drop-shadow-sm"
                >
                  Access your account
                </button>
              </div>
              ...
              <div className="mt-6 grid grid-cols-2 gap-4">
                <Button
                  ...
                  onClick={() => window.location.href = `${import.meta.env.VITE_API_URL || 'https://ai-pos-system.onrender.com'}/oauth2/authorization/google`}
                >
                ...
                <Button
                  ...
                  variant="outline"
                  className="w-full h-12 flex items-center justify-center gap-2 hover:bg-background/90 font-medium border-foreground/20 bg-background/50 shadow-sm backdrop-blur-sm"
                  onClick={() => window.location.href = `${import.meta.env.VITE_API_URL || 'https://ai-pos-system.onrender.com'}/oauth2/authorization/github`}
                >
